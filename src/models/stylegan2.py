...
def normalize_mean_std(x, dim=1, eps=1e-8):
    return (x-x.mean(dim=dim, keepdim=True)) * (x.square().mean(dim=dim, keepdim=True) + eps).rsqrt()
...
class MappingNetwork(torch.nn.Module):
    def __init__(
            ...
            effective_num_samples_lambda, ## applying effective no of samples based noise
            ...
            add_emb_noise=False, #Whether to add a small noise to class embeddings
            emb_noise_std = 0,
    ):
        super().__init__()
        self.z_dim = z_dim
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, mode="train"):
        ...
        if self.c_dim > 0:
            ...
            x = torch.cat([x, normalize_2nd_moment(y)], dim=1) if x is not None else normalize_2nd_moment(y)
...
class Discriminator(torch.nn.Module):
    def __init__(
       ...
       # linear layer for adversarial training
        ...
        if self.d_cond_mtd == "AC":
            ...
            self.linear2 = FullyConnectedLayer(num_classes, channels_dict[4], bias=True)
        elif self.d_cond_mtd == "SPD":
            ...
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=d_embed_dim, num_ws=None, w_avg_beta=None, num_layers=1, **mapping_kwargs)
        else:
            pass
    ...
    def forward(self, img, label, eval=False, adc_fake=False, update_emas=False, is_real=False, **block_kwargs):
        _ = update_emas # unused
        ...
        h = self.b4(x, img) ## (batch_size, channels_dict[4]=512)
        ...
        if self.d_cond_mtd == "AC":
            ...
            adv_output = adv_output + torch.sum(torch.mul(self.linear2(oh_label.to(torch.float32)), h), 1)
        elif self.d_cond_mtd == "SPD":
            embed = self.linear1(h) ## (batch_size, cmap_dim=512)
            cmap = self.mapping(None, oh_label) ## (batch_size, cmap_dim=512)
            adv_output = (embed * cmap).sum(dim=1, keepdim=True) * (1 / np.sqrt(self.cmap_dim)) ## (batch_size, 1)
        elif self.d_cond_mtd in ["2C", "D2DCE"]:
            embed = self.linear2(h)
            proxy = self.mapping(None, oh_label)
            if self.normalize_d_embed:
