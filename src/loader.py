...
def load_worker(local_rank, cfgs, gpus_per_node, run_name, hdf5_path_train, hdf5_path_eval):
    # -----------------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------------
    is_valid_dataset = cfgs.DATA.name in ["iNat19", "imagenet_lt", "CIFAR10_LT", "Imgnet_carniv", "AnimalFace_FS"]
    if cfgs.RUN.train or cfgs.RUN.intra_class_fid or cfgs.RUN.GAN_train or cfgs.RUN.GAN_test or is_valid_dataset:
        if local_rank == 0:
    ...
    ## This is to calculate the effective number of samples based Noise or KLDiv loss for CA.
    if  cfgs.MODEL.effective_number_samples_beta > 0:
        beta = cfgs.MODEL.effective_number_samples_beta
        n = train_dataloader.dataset.img_num_list
        En = np.array([(1 - beta**x) / (1 - beta) for x in n]) # Effective Number of Samples
        K = cfgs.DATA.num_classes / (np.sum(1/En))
        effective_num_samples_lambda = torch.tensor(K/En, dtype=torch.float32).to(local_rank).reshape(-1,1) 
    else:
        effective_num_samples_lambda = torch.tensor([1]*cfgs.DATA.num_classes, dtype=torch.float32).to(local_rank).reshape(-1,1)
    ...
    Gen, Gen_mapping, Gen_synthesis, Dis, Gen_ema, Gen_ema_mapping, Gen_ema_synthesis, ema =\
        model.load_generator_discriminator(DATA=cfgs.DATA,
                                           ...
                                           RUN=cfgs.RUN,
                                           effective_num_samples_lambda = effective_num_samples_lambda,
                                           device=local_rank,
                                           logger=logger)
    ...
    if cfgs.RUN.vis_fake_images:
        if global_rank == 0:
            print(""), logger.info("-" * 80)
        worker.visualize_fake_images_lt(num_cols=num_cols, current_step=best_step)
    
    if cfgs.RUN.vis_emb_umap:
        if global_rank == 0:
            print(""), logger.info("-" * 80)
        worker.visualize_embedding_umap(current_step=best_step)
    ...
    if cfgs.RUN.tsne_analysis:
        if global_rank == 0:
            print(""), logger.info("-" * 80)
        worker.run_w_space_tsne(dataloader=eval_dataloader)

    if cfgs.RUN.intra_class_fid:
        if global_rank == 0:
            print(""), logger.info("-" * 80)
        worker.calculate_intra_class_fid(dataset=eval_dataset, ref_data=cfgs.RUN.ref_dataset)
