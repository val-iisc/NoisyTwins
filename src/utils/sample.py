...
def sample_y(y_sampler, batch_size, num_classes, device):
    ...
    elif y_sampler == "totally_random_barlow":
        ## Keeping same labels for both twins in NoisyTwins.
        y_fake = torch.randint(low=0, high=num_classes, size=(batch_size//2, ), dtype=torch.long, device=device)
        y_fake = torch.cat([y_fake, y_fake], dim=0)
...
def generate_images(z_prior, truncation_factor, batch_size, z_dim, num_classes, y_sampler, radius, generator, discriminator,
                    ...
                    stylegan_update_emas, cal_trsp_cost, mode="train"):
    if is_train:
    ...
    zs, fake_labels, zs_eps = sample_zy(z_prior=z_prior,
                                        ...
                                        radius=radius,
                                        device=device)
    ## copy the z vectors of first twin to other twin for NoisyTwins Regularizer.
    if y_sampler == "totally_random_barlow":
        batch_size = zs.shape[0]
        zs = torch.cat([zs[:batch_size//2], zs[:batch_size//2]], dim=0)
    batch_size = fake_labels.shape[0]
...
def stylegan_generate_images(zs, fake_labels, num_classes, style_mixing_p, update_emas, 
                             generator_mapping, generator_synthesis, truncation_psi, truncation_cutoff, mode="train"):
    one_hot_fake_labels = F.one_hot(fake_labels, num_classes=num_classes)
    if truncation_psi == -1:
        ws = generator_mapping(zs, one_hot_fake_labels, truncation_psi=1, update_emas=update_emas, mode=mode)
    else:
        ws = generator_mapping(zs, one_hot_fake_labels, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas, mode=mode)
    if style_mixing_p > 0:
        cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
        cutoff = torch.where(torch.rand([], device=ws.device) < style_mixing_p, cutoff, torch.full_like(cutoff, ws.shape[1]))
        ws[:, cutoff:] = generator_mapping(torch.randn_like(zs), one_hot_fake_labels, update_emas=False, mode=mode)[:, cutoff:]
