...
class LeCamEMA(object):
    ...
    def __init__(self, init=0, decay=0.9, start_iter=0):
        self.G_loss = init
...
class EmbNormEMA(object):
    # Simple wrapper that applies EMA to embedding norms of Stylegan.
    def __init__(self, Gen_init_norm, Dis_init_norm, decay=0.9, start_iter=0):
        with torch.no_grad():
            self.Gen_norm = Gen_init_norm ## shape (1 x n_classes)
            self.Dis_norm = Dis_init_norm ## shape (1 x n_classes)
            self.decay = decay
            self.start_itr = start_iter

    def update(self, cur, labels, mode, itr):
        if itr < self.start_itr:
            decay = 0.0
        else:
            decay = self.decay
        ...
        if mode == "Gen_norm":
            for i, label in enumerate(labels):
                self.Gen_norm[label,0] = self.Gen_norm[label,0]*decay + cur[i,0]*(1 - decay)
        elif mode == "Dis_norm":
            for i, label in enumerate(labels):
                self.Dis_norm[label,0] = self.Dis_norm[label,0]*decay + cur[i,0]*(1 - decay)
